...
void setup() {
  ...
  sun = new Planet(5, 0, 0,sun_rad,0 , sunTexture);
  
  Mercury = new Planet(rad_scale(Mercury_rad),dis_scale(Mercury_dis  ),inv_scale2(Mercury_orb ) ,inv_scale(Mercury_rot ),Mercury_inc, textures[0]);
  
  
  Venus = new Planet(rad_scale(Venus_rad),dis_scale(Venus_dis  ),inv_scale2(Venus_orb ) ,inv_scale(Venus_rot ),Venus_inc, textures[1]);
  
  
  Earth = new Planet(rad_scale(Earth_rad),dis_scale(Earth_dis  ),inv_scale2(Earth_orb ) ,inv_scale(Earth_rot ),Earth_inc, textures[2]);
  
  
  Mars = new Planet(rad_scale(Mars_rad),dis_scale(Mars_dis  ),inv_scale2(Mars_orb ) ,inv_scale(Mars_rot ),Mars_inc, textures[3]);
  
  
  Jupiter = new Planet(rad_scale(Jupiter_rad),dis_scale(Jupiter_dis  ),inv_scale2(Jupiter_orb ) ,inv_scale(Jupiter_rot ),Jupiter_inc, textures[4]);
  
  
  Saturn = new Planet(rad_scale(Saturn_rad),dis_scale(Saturn_dis  ),inv_scale2(Saturn_orb ) ,inv_scale(Saturn_rot ),Saturn_inc, textures[5]);
  
  
  Uranus = new Planet(rad_scale(Uranus_rad),dis_scale(Uranus_dis  ),inv_scale2(Uranus_orb ) ,inv_scale(Uranus_rot ),Uranus_inc, textures[6]);
  
  
  Neptune = new Planet(rad_scale(Neptune_rad),dis_scale(Neptune_dis  ),inv_scale2(Neptune_orb ) ,inv_scale(Neptune_rot ),Neptune_inc, textures[7]);
  
  
  Pluto = new Planet(rad_scale(Pluto_rad),dis_scale(Pluto_dis  ),inv_scale2(Pluto_orb ) ,inv_scale(Pluto_rot ),Pluto_inc, textures[8]);
